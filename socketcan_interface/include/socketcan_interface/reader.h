...
public:
    ...
    void listen(CommInterfaceSharedPtr interface){
        boost::mutex::scoped_lock lock(mutex_);
        listener_ = interface->createMsgListener(CommInterface::FrameDelegate(CommInterface::createFrameDelegate(this, &BufferedReader::handleFrame)));
        buffer_.clear();
    }
    void listen(CommInterfaceSharedPtr interface, const Frame::Header& h){
        boost::mutex::scoped_lock lock(mutex_);
        listener_ = interface->createMsgListener(h, CommInterface::FrameDelegate(CommInterface::createFrameDelegate(this, &BufferedReader::handleFrame)));
        buffer_.clear();
    }
